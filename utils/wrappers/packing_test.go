...
func TestPackerPackFixedBytes(t *testing.T) {
	...
	p := Packer{MaxSize: 4}
	p.PackFixedBytes([]byte("Cryft"))
	require.False(p.Errored())
	require.NoError(p.Err)
	require.Equal([]byte("Cryft"), p.Bytes)
...
func TestPackerUnpackFixedBytes(t *testing.T) {
	...
	p := Packer{Bytes: []byte("Cryft")}
	require.Equal([]byte("Cryft"), p.UnpackFixedBytes(4))
	require.False(p.Errored())
	require.NoError(p.Err)
...
func TestPackerPackBytes(t *testing.T) {
	...
	p := Packer{MaxSize: 8}
	p.PackBytes([]byte("Cryft"))
	require.False(p.Errored())
	require.NoError(p.Err)
	require.Equal([]byte("\x00\x00\x00\x04Cryft"), p.Bytes)
...
func TestPackerUnpackBytes(t *testing.T) {
	...
	p := Packer{Bytes: []byte("\x00\x00\x00\x04Cryft")}
	require.Equal([]byte("Cryft"), p.UnpackBytes())
	require.False(p.Errored())
	require.NoError(p.Err)
...
func TestPackerUnpackLimitedBytes(t *testing.T) {
	...
	p := Packer{Bytes: []byte("\x00\x00\x00\x04Cryft")}
	require.Equal([]byte("Cryft"), p.UnpackLimitedBytes(10))
	require.False(p.Errored())
	require.NoError(p.Err)
	require.Equal(8, p.Offset)
...
func TestPackerUnpackString(t *testing.T) {
	...
	p := Packer{Bytes: []byte("\x00\x04Cryft")}

	require.Equal("Cryft", p.UnpackStr())
	require.False(p.Errored())
	require.NoError(p.Err)
...
func TestPackerUnpackLimitedString(t *testing.T) {
	...
	p := Packer{Bytes: []byte("\x00\x04Cryft")}
	require.Equal("Cryft", p.UnpackLimitedStr(10))
	require.False(p.Errored())
	require.NoError(p.Err)
	require.Equal(6, p.Offset)
