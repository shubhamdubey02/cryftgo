...
`memdb` is useful for fast one-off testing, not for running an actual node (on Mustang or Mainnet).
...
standard networks (Mainnet, Mustang Testnet), or when `--genesis-content` is
...
`"https://*.cryft.network https://*.cryft-test.network"`
...
- `--network-id=mustang` -&gt; Connect to the Mustang test-network.
- `--network-id=testnet` -&gt; Connect to the current test-network. (Right now, this is Mustang.)
...
not be disabled on public networks (`Mustang` and `Mainnet`).
...
:::note
Some of these parameters can only be set on a local or private network, not on Mustang Testnet or Mainnet
:::
...
Transaction fee, in nCRYFT, for transactions that create new assets. Defaults to
`10000000` nCRYFT (.01 CRYFT) per transaction. This can only be changed on a local
network.

#### `--create-subnet-tx-fee` (int)

Transaction fee, in nCRYFT, for transactions that create new Subnets. Defaults to
`1000000000` nCRYFT (1 CRYFT) per transaction. This can only be changed on a local
network.

#### `--create-blockchain-tx-fee` (int)

Transaction fee, in nCRYFT, for transactions that create new blockchains.
Defaults to `1000000000` nCRYFT (1 CRYFT) per transaction. This can only be
changed on a local network.

#### `--transform-subnet-tx-fee` (int)

Transaction fee, in nCRYFT, for transactions that transform Subnets. Defaults to
`1000000000` nCRYFT (1 CRYFT) per transaction. This can only be changed on a local network.

#### `--add-primary-network-validator-fee` (int)

Transaction fee, in nCRYFT, for transactions that add new primary network validators. Defaults to 0.
This can only be changed on a local network.

#### `--add-primary-network-delegator-fee` (int)

Transaction fee, in nCRYFT, for transactions that add new primary network delegators. Defaults to 0.
This can only be changed on a local network.

#### `--add-subnet-validator-fee` (int)

Transaction fee, in nCRYFT, for transactions that add new Subnet validators.
Defaults to `10000000` nCRYFT (.01 CRYFT).

#### `--add-subnet-delegator-fee` (int)

Transaction fee, in nCRYFT, for transactions that add new Subnet delegators.
Defaults to `10000000` nCRYFT (.01 CRYFT).

#### `--min-delegator-stake` (int)

The minimum stake, in nCRYFT, that can be delegated to a validator of the Primary Network.

Defaults to `25000000000` (25 CRYFT) on Mainnet. Defaults to `5000000` (.005
CRYFT) on Test Net. This can only be changed on a local network.
...
The minimum stake, in nCRYFT, required to validate the Primary Network. This can
only be changed on a local network.

Defaults to `2000000000000` (2,000 CRYFT) on Mainnet. Defaults to `5000000` (.005 CRYFT) on Test Net.
...
The maximum stake, in nCRYFT, that can be placed on a validator on the primary
network. Defaults to `3000000000000000` (3,000,000 CRYFT) on Mainnet. This
...
The maximum stake supply, in nCRYFT, that can be placed on a validator. Defaults
to `720,000,000,000,000,000` nCRYFT. This can only be changed on a local network.

#### `--tx-fee` (int)

The required amount of nCRYFT to be burned for a transaction to be valid on the
...
`1,000,000` nCRYFT per transaction.
