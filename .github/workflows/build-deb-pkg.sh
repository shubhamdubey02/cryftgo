...
OK=$(cp ./build/cryftgo "$AVALANCHE_BUILD_BIN_DIR")
if [[ $OK -ne 0 ]]; then
  exit "$OK";
fi
...
dpkg-deb --build debian "cryftgo-$TAG-$ARCH.deb"
aws s3 cp "cryftgo-$TAG-$ARCH.deb" "s3://${BUCKET}/linux/debs/ubuntu/$RELEASE/$ARCH/"
