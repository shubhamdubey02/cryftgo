...
# Polls CryftGo until it's healthy. When it is,
# sets SUCCESS to 0 and returns. If CryftGo
...
rm -rf /var/lib/cryftgo 2>/dev/null || true # Do || true to ignore error if files dont exist yet
...
mkdir -p /var/lib/cryftgo/db
tar -zxf /opt/"$DB_FILE"*-tar.gz -C /var/lib/cryftgo/db
...
containerID=$(docker run --name="net_outage_simulation" --memory="12g" --memory-reservation="11g" --cpus="6.0" --net=controlled-net -p 9650:9650 -p 9651:9651 -v /var/lib/cryftgo/db:/db -d cryftplatform/cryftgo:latest /cryftgo/build/cryftgo --db-dir /db --http-host=0.0.0.0)
