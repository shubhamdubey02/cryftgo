...
- Changed the default value of `--network-allow-private-ips` to `false` when the `--network-id` is either `mustang` or `mainnet`
...
- Activated non-CRYFT `ImportTx`/`ExportTx`s to/from the P-chain
...
### [Coreth](https://medium.com/avalanchecryft/apricot-phase-6-native-asset-call-deprecation-a7b7a77b850a)
...
The changes in `v1.8.x` go into effect at 4 PM EDT on September 6th, 2022 on both Mustang and Mainnet. You should upgrade your node before the changes go into effect, otherwise they may experience loss of uptime.
...
The changes in the upgrade go into effect at **4 PM EDT on September 6th, 2022** on both Mustang and Mainnet. You should upgrade your node before the changes go into effect, otherwise they may experience loss of uptime.
...
- Updated API names used to enable services in `eth-api` config flag. Prior names are supported but deprecated, please update configurations [accordingly](https://docs.cryft.network/nodes/maintain/chain-config-flags#c-chain-configs)
...
**These API format changes are breaking changes. https://api.cryft.network and https://api.cryft-test.network have been updated with this format. If you are using CryftGo APIs in your code, please ensure you have updated to the latest versions. See  https://docs.cryft.network/apis/cryftgo/cb58-deprecation for details about the CB58 removal.**
...
- Added `--stake-supply-cap` which defaults to `720,000,000,000,000,000` nCRYFT.
...
- Enforced that `--staking-enabled=false` can not be specified on public networks (`Mustang` and `Mainnet`).
...
- Added the ability to export/import CRYFT to/from the C-chain.

### Coreth

- Enabled multiple `AtomicTx`s to be issued per block.
- Added the ability to export/import CRYFT to/from the P-chain.
