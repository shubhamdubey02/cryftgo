...
var _ = e2e.DescribeCChain("[Interchain Workflow]", func() {
	...
	const txAmount = 10 * units.Cryft // Arbitrary amount to send and transfer
	...
	ginkgo.It("should ensure that funds can be transferred from the C-Chain to the X-Chain and the P-Chain", func() {
		...
		ginkgo.By("exporting CRYFT from the C-Chain to the X-Chain", func() {
		...
		ginkgo.By("importing CRYFT from the C-Chain to the X-Chain", func() {
		...
		ginkgo.By("exporting CRYFT from the C-Chain to the P-Chain", func() {
			_, err := cWallet.IssueExportTx(
		...
		ginkgo.By("importing CRYFT from the C-Chain to the P-Chain", func() {
