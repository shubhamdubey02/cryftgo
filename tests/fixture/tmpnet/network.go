...
func (n *Network) Start(ctx context.Context, w io.Writer) error {
	...
	if _, err := fmt.Fprintf(w, "\nMetrics: https://grafana-experimental.cryft-dev.network/d/kBQpRdWnk/avalanche-main-dashboard?&var-filter=network_uuid%%7C%%3D%%7C%s&var-filter=is_ephemeral_node%%7C%%3D%%7Cfalse&from=%d&to=now\n", n.UUID, startTime.UnixMilli()); err != nil {
		return err
	}
