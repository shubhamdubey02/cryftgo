...
const (
	...
	// Arbitrarily large amount of CRYFT to fund keys on the X-Chain for testing
	defaultFundedKeyXChainAmount = 30 * units.MegaCryft
)

var (
	// Arbitrarily large amount of CRYFT (10^12) to fund keys on the C-Chain for testing
...
func NewTestGenesis(
	...
	// Ensure the total stake allows a MegaCryft per staker
	totalStake := uint64(len(initialStakers)) * units.MegaCryft
	...
	for xChainAddress, balance := range xChainBalances {
		cryftAddr, err := address.Format("X", constants.GetHRP(networkID), xChainAddress[:])
		if err != nil {
