...
import (
	...
	xchaintxs "github.com/shubhamdubey02/cryftgo/vms/avm/txs"
	pchaintxs "github.com/shubhamdubey02/cryftgo/vms/platformvm/txs"
)
...
func validateInitialStakedFunds(config *Config) error {
	...
	for _, allocation := range config.Allocations {
		...
		// ethAddrs could claim to the same cryftAddr.
		allocationSet.Add(allocation.CRYFTAddr)
	}
...
func FromConfig(config *Config) ([]byte, ids.ID, error) {
	...
	}
	{
		...
		for _, allocation := range xAllocations {
			addr, err := address.FormatBech32(hrp, allocation.CRYFTAddr.Bytes())
			if err != nil {
				return nil, ids.ID{}, err
			}

			cryft.InitialState["fixedCap"] = append(cryft.InitialState["fixedCap"], avm.Holder{
		...
		var err error
		cryft.Memo, err = formatting.Encode(defaultEncoding, memoBytes)
		if err != nil {
			return nil, ids.Empty, fmt.Errorf("couldn't parse memo bytes to string: %w", err)
		}
		avmArgs.GenesisData = map[string]avm.AssetDefinition{
			"CRYFT": cryft, // The AVM starts out with one asset: CRYFT
		}
	}
	...
	cryftAssetID, err := CRYFTAssetID(bytes)
	if err != nil {
		return nil, ids.ID{}, fmt.Errorf("couldn't generate CRYFT asset ID: %w", err)
	}
	...
	for _, allocation := range config.Allocations {
		if initiallyStaked.Contains(allocation.CRYFTAddr) {
	...
	return genesisBytes, cryftAssetID, nil
}
...
func CRYFTAssetID(avmGenesisBytes []byte) (ids.ID, error) {
