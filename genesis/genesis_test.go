...
func TestGenesisFromFile(t *testing.T) {
	tests := map[string]struct {
		...
		"mustang": {
			networkID:    constants.MustangID,
			customConfig: customGenesisConfigJSON,
			expectedErr:  errOverridesStandardNetworkConfig,
		},
		"mustang (with custom specified)": {
			networkID:    constants.MustangID,
		...
		t.Run(name, func(t *testing.T) {
			...
			if len(test.customConfig) == 0 {
				...
				case constants.TestnetID:
					genBytes, err = json.Marshal(&MustangConfig)
					require.NoError(err)
...
func TestCRYFTAssetID(t *testing.T) {
	tests := []struct {
		networkID  uint32
		expectedID string
	}{
	...
	for _, test := range tests {
		t.Run(constants.NetworkIDToNetworkName[test.networkID], func(t *testing.T) {
			...
			config := GetConfig(test.networkID)
			_, cryftAssetID, err := FromConfig(config)
			require.NoError(err)

			require.Equal(
				test.expectedID,
				cryftAssetID.String(),
				"CRYFT assetID with networkID %d mismatch",
				test.networkID,
			)
		})
	}
