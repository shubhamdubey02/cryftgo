...
type AddressTxsIndexer interface {
	...
	Accept(
		txID ids.ID,
		inputUTXOs []*cryft.UTXO,
		outputUTXOs []*cryft.UTXO,
	) error
...
func (i *indexer) Accept(txID ids.ID, inputUTXOs []*cryft.UTXO, outputUTXOs []*cryft.UTXO) error {
	utxos := inputUTXOs
	...
	for _, utxo := range utxos {
		out, ok := utxo.Out.(cryft.Addressable)
		if !ok {
...
func (*noIndexer) Accept(ids.ID, []*cryft.UTXO, []*cryft.UTXO) error {
	return nil
}
