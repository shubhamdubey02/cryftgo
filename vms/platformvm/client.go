...
type Client interface {
	...
	// GetCurrentSupply returns an upper bound on the supply of CRYFT in the system along with the P-chain height
	...
	// GetStake returns the amount of nCRYFT that [addrs] have cumulatively
	...
	// GetMinStake returns the minimum staking amount in nCRYFT for validators
	// and delegators respectively
	GetMinStake(ctx context.Context, subnetID ids.ID, options ...rpc.Option) (uint64, uint64, error)
	// GetTotalStake returns the total amount (in nCRYFT) staked on the network
