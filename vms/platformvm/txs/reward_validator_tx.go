...
// receives the staked CRYFT as well as a validating reward.
//
// If this transaction is accepted and the next block accepted is an Abort
// block, the validator is removed and the address that the validator specified
// receives the staked CRYFT but no reward.
...
func (*RewardValidatorTx) Outputs() []*cryft.TransferableOutput {
	return nil
}
