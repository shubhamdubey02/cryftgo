...
func TestRewardValidatorTxExecuteOnCommit(t *testing.T) {
	...
	// Get old balances
	oldBalance, err := cryft.GetBalance(env.state, stakeOwners)
	require.NoError(err)
	...
	onCommitBalance, err := cryft.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardValidatorTxExecuteOnAbort(t *testing.T) {
	...
	onAbortBalance, err := cryft.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardDelegatorTxExecuteOnCommitPreDelegateeDeferral(t *testing.T) {
	...
	oldVdrBalance, err := cryft.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	oldDelBalance, err := cryft.GetBalance(env.state, delDestSet)
	require.NoError(err)
	...
	commitVdrBalance, err := cryft.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	...
	commitDelBalance, err := cryft.GetBalance(env.state, delDestSet)
	require.NoError(err)
...
func TestRewardDelegatorTxExecuteOnCommitPostDelegateeDeferral(t *testing.T) {
	...
	preCortinaVdrRewardUTXOID := &cryft.UTXOID{
		TxID:        delTx.ID(),
	...
	onCommitVdrDelRewardUTXOID := &cryft.UTXOID{
		TxID:        vdrTx.ID(),
	...
	onAbortVdrDelRewardUTXOID := &cryft.UTXOID{
		TxID:        vdrTx.ID(),
		OutputIndex: numVdrStakeUTXOs + 1,
	}
