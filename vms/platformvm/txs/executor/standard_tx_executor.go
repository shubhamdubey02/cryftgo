...
func (e *StandardTxExecutor) CreateChainTx(tx *txs.CreateChainTx) error {
	...
	if err := cryft.VerifyMemoFieldLength(tx.Memo, isDurangoActive); err != nil {
		return err
	}
	...
	// Consume the UTXOS
	cryft.Consume(e.State, tx.Ins)
	// Produce the UTXOS
	cryft.Produce(e.State, txID, tx.Outs)
...
func (e *StandardTxExecutor) ImportTx(tx *txs.ImportTx) error {
	...
	if e.Bootstrapped.Get() && !e.Config.PartialSyncPrimaryNetwork {
		...
		utxos := make([]*cryft.UTXO, len(tx.Ins)+len(tx.ImportedInputs))
		...
		ins := make([]*cryft.TransferableInput, len(tx.Ins)+len(tx.ImportedInputs))
		copy(ins, tx.Ins)
...
func (e *StandardTxExecutor) ExportTx(tx *txs.ExportTx) error {
	...
	outs := make([]*cryft.TransferableOutput, len(tx.Outs)+len(tx.ExportedOutputs))
	copy(outs, tx.Outs)
...
func (e *StandardTxExecutor) TransformSubnetTx(tx *txs.TransformSubnetTx) error {
	...
	if err := e.Backend.FlowChecker.VerifySpend(
		...
		tx.Outs,
		baseTxCreds,
		// Invariant: [tx.AssetID != e.Ctx.CRYFTAssetID]. This prevents the first
...
func (e *StandardTxExecutor) BaseTx(tx *txs.BaseTx) error {
	...
	if err := cryft.VerifyMemoFieldLength(tx.Memo, true /*=isDurangoActive*/); err != nil {
		return err
	}
