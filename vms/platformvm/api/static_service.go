...
import (
	...
	"github.com/shubhamdubey02/cryftgo/vms/platformvm/txs/txheap"
...
func (*StaticService) BuildGenesis(_ *http.Request, args *BuildGenesisArgs, reply *BuildGenesisReply) error {
	...
	for i, apiUTXO := range args.UTXOs {
		...
		utxo := cryft.UTXO{
			...
			Asset: cryft.Asset{ID: args.CryftAssetID},
		...
		if apiUTXO.Locktime > args.Time {
			utxo.Out = &stakeable.LockOut{
				...
				TransferableOut: utxo.Out.(cryft.TransferableOut),
			}
		}
	...
	for _, vdr := range args.Validators {
		weight := uint64(0)
		stake := make([]*cryft.TransferableOutput, len(vdr.Staked))
