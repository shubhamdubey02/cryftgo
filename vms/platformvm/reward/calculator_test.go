...
func TestLongerDurationBonus(t *testing.T) {
	...
	shortBalance := units.KiloCryft
	for i := 0; i < int(totalDuration/shortDuration); i++ {
		r := c.Calculate(shortDuration, shortBalance, 359*units.MegaCryft+shortBalance)
		shortBalance += r
	}
	reward := c.Calculate(totalDuration%shortDuration, shortBalance, 359*units.MegaCryft+shortBalance)
	shortBalance += reward

	longBalance := units.KiloCryft
	longBalance += c.Calculate(totalDuration, longBalance, 359*units.MegaCryft+longBalance)
...
func TestRewards(t *testing.T) {
	...
	tests := []struct {
		...
		{ // (720M - 360M) * (1M / 360M) * 12%
			...
			stakeAmount:    units.MegaCryft,
			existingAmount: 360 * units.MegaCryft,
			expectedReward: 120 * units.KiloCryft,
		},
		{ // (720M - 400M) * (1M / 400M) * 12%
			duration:       defaultMaxStakingDuration,
			stakeAmount:    units.MegaCryft,
			existingAmount: 400 * units.MegaCryft,
			expectedReward: 96 * units.KiloCryft,
		},
		{ // (720M - 400M) * (2M / 400M) * 12%
			duration:       defaultMaxStakingDuration,
			stakeAmount:    2 * units.MegaCryft,
			existingAmount: 400 * units.MegaCryft,
			expectedReward: 192 * units.KiloCryft,
		},
