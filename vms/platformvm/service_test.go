...
import (
	...
	"github.com/shubhamdubey02/cryftgoftgo/vms/platformvm/block/builder"
	...
	vmkeystore "github.com/shubhamdubey02/cryftgoftgo/vms/components/keystore"
	pchainapi "github.com/shubhamdubey02/cryftgoftgo/vms/platformvm/api"
...
func defaultService(t *testing.T) (*Service, *mutableSharedMemory, *txstest.Builder) {
	...
	return &Service{
		vm:          vm,
		addrManager: cryft.NewAddressManager(vm.ctx),
...
func TestGetTxStatus(t *testing.T) {
	...
	utxo := &cryft.UTXO{
		...
		Asset: cryft.Asset{ID: service.vm.ctx.CRYFTAssetID},
...
func TestGetBalance(t *testing.T) {
	...
	genesis, _ := defaultGenesis(t, service.vm.ctx.CRYFTAssetID)
...
func TestGetStake(t *testing.T) {
	...
	for i, validator := range genesis.Validators {
		...
		var output cryft.TransferableOutput
	...
	outputs := make([]cryft.TransferableOutput, 2)
	for i := range outputs {
	...
	// Unmarshal
	outputs = make([]cryft.TransferableOutput, 3)
	for i := range outputs {
		outputBytes, err := formatting.Decode(args.Encoding, response.Outputs[i])
		require.NoError(err)
		_, err = txs.Codec.Unmarshal(outputBytes, &outputs[i])
