...
import (
	...
	"github.com/shubhamdubey02/cryftgo/vms/components/keystore"
	...
	platformapi "github.com/shubhamdubey02/cryftgo/vms/platformvm/api"
)
...
type Service struct {
	vm                    *VM
	addrManager           cryft.AddressManager
...
func (s *Service) ExportKey(_ *http.Request, args *ExportKeyArgs, reply *ExportKeyReply) error {
	...
	address, err := cryft.ParseServiceAddress(s.addrManager, args.Address)
	if err != nil {
...
// Note: We explicitly duplicate CRYFT out of the maps to ensure backwards
// compatibility.
type GetBalanceResponse struct {
	// Balance, in nCRYFT, of the address
...
func (s *Service) GetBalance(_ *http.Request, args *GetBalanceRequest, response *GetBalanceResponse) error {
	...
	addrs, err := cryft.ParseServiceAddresses(s.addrManager, args.Addresses)
	if err != nil {
		return err
	}
	...
	utxos, err := cryft.GetAllUTXOs(s.vm.state, addrs)
	if err != nil {
...
utxoFor:
	...
	response.Balance = response.Balances[s.vm.ctx.CRYFTAssetID]
	response.Unlocked = response.Unlockeds[s.vm.ctx.CRYFTAssetID]
	response.LockedStakeable = response.LockedStakeables[s.vm.ctx.CRYFTAssetID]
	response.LockedNotStakeable = response.LockedNotStakeables[s.vm.ctx.CRYFTAssetID]
	return nil
}
...
func (s *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, response *api.GetUTXOsReply) error {
	...
	addrSet, err := cryft.ParseServiceAddresses(s.addrManager, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = cryft.ParseServiceAddress(s.addrManager, args.StartIndex.Address)
		if err != nil {
	...
	if sourceChain == s.vm.ctx.ChainID {
		utxos, endAddr, endUTXOID, err = cryft.GetPaginatedUTXOs(
			s.vm.state,
			addrSet,
			startAddr,
			startUTXO,
		...
		utxos, endAddr, endUTXOID, err = cryft.GetAtomicUTXOs(
...
func (s *Service) GetStakingAssetID(_ *http.Request, args *GetStakingAssetIDArgs, response *GetStakingAssetIDResponse) error {
	...
	if args.SubnetID == constants.PrimaryNetworkID {
		response.AssetID = s.vm.ctx.CRYFTAssetID
		return nil
	}
...
// GetCurrentSupply returns an upper bound on the supply of CRYFT in the system
...
type GetStakeReply struct {
	...
	// Each is of type cryft.TransferableOutput
	Outputs []string `json:"stakedOutputs"`
	// Encoding of [Outputs]
	Encoding formatting.Encoding `json:"encoding"`
}

// GetStake returns the amount of nCRYFT that [args.Addresses] have cumulatively
// staked on the Primary Network.
//
// This method assumes that each stake output has only owner
// This method assumes only CRYFT can be staked
...
func (s *Service) GetStake(_ *http.Request, args *GetStakeArgs, response *GetStakeReply) error {
	...
	var (
		...
		stakedOuts        []cryft.TransferableOutput
	)
	...
	response.Staked = response.Stakeds[s.vm.ctx.CRYFTAssetID]
...
// GetMinStake returns the minimum staking amount in nCRYFT.
...
func getStakeHelper(tx *txs.Tx, addrs set.Set[ids.ShortID], totalAmountStaked map[ids.ID]uint64) []cryft.TransferableOutput {
	...
	stake := staker.Stake()
	stakedOuts := make([]cryft.TransferableOutput, 0, len(stake))
