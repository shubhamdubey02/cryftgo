...
type Chain interface {
	...
	AddRewardUTXO(txID ids.ID, utxo *cryft.UTXO)
...
type State interface {
	...
	GetRewardUTXOs(txID ids.ID) ([]*cryft.UTXO, error)
...
type state struct {
	...
	addedRewardUTXOs map[ids.ID][]*cryft.UTXO            // map of txID -> []*UTXO
	rewardUTXOsCache cache.Cacher[ids.ID, []*cryft.UTXO] // txID -> []*UTXO
	rewardUTXODB     database.Database

	modifiedUTXOs map[ids.ID]*cryft.UTXO // map of modified UTXOID -> *UTXO if the UTXO is nil, it has been removed
	utxoDB        database.Database
	utxoState     cryft.UTXOState
...
func newState(
	...
	rewardUTXOsCache, err := metercacher.New[ids.ID, []*cryft.UTXO](
		"reward_utxos_cache",
		metricsReg,
		&cache.LRU[ids.ID, []*cryft.UTXO]{Size: execCfg.RewardUTXOsCacheSize},
	)
	if err != nil {
		return nil, err
	}
	...
	utxoState, err := cryft.NewMeteredUTXOState(utxoDB, txs.GenesisCodec, metricsReg, execCfg.ChecksumsEnabled)
	if err != nil {
		return nil, err
	}
	...
	return &state{
		...
		addedRewardUTXOs: make(map[ids.ID][]*cryft.UTXO),
		rewardUTXODB:     rewardUTXODB,
		rewardUTXOsCache: rewardUTXOsCache,

		modifiedUTXOs: make(map[ids.ID]*cryft.UTXO),
		utxoDB:        utxoDB,
		utxoState:     utxoState,
...
func (s *state) GetRewardUTXOs(txID ids.ID) ([]*cryft.UTXO, error) {
...
func (s *state) AddRewardUTXO(txID ids.ID, utxo *cryft.UTXO) {
	s.addedRewardUTXOs[txID] = append(s.addedRewardUTXOs[txID], utxo)
}

func (s *state) GetUTXO(utxoID ids.ID) (*cryft.UTXO, error) {
...
func (s *state) AddUTXO(utxo *cryft.UTXO) {
...
func (s *state) syncGenesis(genesisBlk block.Block, genesis *genesis.Genesis) error {
	...
	for _, chain := range genesis.Chains {
		...
		if unsignedChain.NetworkID != s.ctx.NetworkID {
			return cryft.ErrWrongNetworkID
		}
