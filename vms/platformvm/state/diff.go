...
type diff struct {
	...
	addedRewardUTXOs map[ids.ID][]*cryft.UTXO

	addedTxs map[ids.ID]*txAndStatus

	// map of modified UTXOID -> *UTXO if the UTXO is nil, it has been removed
	modifiedUTXOs map[ids.ID]*cryft.UTXO
}
...
func (d *diff) AddRewardUTXO(txID ids.ID, utxo *cryft.UTXO) {
	if d.addedRewardUTXOs == nil {
		d.addedRewardUTXOs = make(map[ids.ID][]*cryft.UTXO)
	}
	d.addedRewardUTXOs[txID] = append(d.addedRewardUTXOs[txID], utxo)
}

func (d *diff) GetUTXO(utxoID ids.ID) (*cryft.UTXO, error) {
...
func (d *diff) AddUTXO(utxo *cryft.UTXO) {
	if d.modifiedUTXOs == nil {
		d.modifiedUTXOs = map[ids.ID]*cryft.UTXO{
			utxo.InputID(): utxo,
		}
	} else {
