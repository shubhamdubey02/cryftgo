...
func (w *WalletService) update(utxos []*cryft.UTXO) ([]*cryft.UTXO, error) {
	utxoMap := make(map[ids.ID]*cryft.UTXO, len(utxos))
...
func (w *WalletService) SendMultiple(_ *http.Request, args *SendMultipleArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	fromAddrs, err := cryft.ParseServiceAddresses(w.vm, args.From)
	if err != nil {
	...
	for _, output := range args.Outputs {
		...
		to, err := cryft.ParseServiceAddress(w.vm, output.To)
		if err != nil {
