...
type GetAddressTxsArgs struct {
	...
	// AssetID defaulted to CRYFT if omitted or left blank
...
func (s *Service) GetAddressTxs(_ *http.Request, args *GetAddressTxsArgs, reply *GetAddressTxsReply) error {
	...
	// Parse to address
	address, err := cryft.ParseServiceAddress(s.vm, args.Address)
	if err != nil {
...
func (s *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, reply *api.GetUTXOsReply) error {
	...
	addrSet, err := cryft.ParseServiceAddresses(s.vm, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = cryft.ParseServiceAddress(s.vm, args.StartIndex.Address)
		if err != nil {
...
func (s *Service) GetBalance(_ *http.Request, args *GetBalanceArgs, reply *GetBalanceReply) error {
	...
	addr, err := cryft.ParseServiceAddress(s.vm, args.Address)
	if err != nil {
	...
	utxos, err := cryft.GetAllUTXOs(s.vm.state, addrSet)
	if err != nil {
		return fmt.Errorf("problem retrieving UTXOs: %w", err)
	}

	now := s.vm.clock.Unix()
	reply.UTXOIDs = make([]cryft.UTXOID, 0, len(utxos))
...
func (s *Service) buildCreateAssetTx(args *CreateAssetArgs) (*txs.Tx, ids.ShortID, error) {
	...
	fromAddrs, err := cryft.ParseServiceAddresses(s.vm, args.From)
	if err != nil {
	...
	if amountSpent := amountsSpent[s.vm.feeAssetID]; amountSpent > s.vm.CreateAssetTxFee {
		outs = append(outs, &cryft.TransferableOutput{
			Asset: cryft.Asset{ID: s.vm.feeAssetID},
	...
	for _, holder := range args.InitialHolders {
		addr, err := cryft.ParseServiceAddress(s.vm, holder.Address)
		if err != nil {
			return nil, ids.ShortEmpty, err
		}
	...
	for _, owner := range args.MinterSets {
		...
		minterAddrsSet, err := cryft.ParseServiceAddresses(s.vm, owner.Minters)
		if err != nil {
			return nil, ids.ShortEmpty, err
		}
...
func (s *Service) buildSendMultiple(args *SendMultipleArgs) (*txs.Tx, ids.ShortID, error) {
	...
	if l := len(memoBytes); l > cryft.MaxMemoSize {
		return nil, ids.ShortEmpty, fmt.Errorf("max memo length is %d but provided memo field is length %d", cryft.MaxMemoSize, l)
	...
	for _, output := range args.Outputs {
		...
		to, err := cryft.ParseServiceAddress(s.vm, output.To)
		if err != nil {
	...
	codec := s.vm.parser.Codec()
	cryft.SortTransferableOutputs(outs, codec)
...
func (s *Service) buildMint(args *MintArgs) (*txs.Tx, ids.ShortID, error) {
	...
	to, err := cryft.ParseServiceAddress(s.vm, args.To)
	if err != nil {
...
type ImportArgs struct {
	...
	// Address receiving the imported CRYFT
	To string `json:"to"`
}

// Import imports an asset to this chain from the P/C-Chain.
// The CRYFT must have already been exported from the P/C-Chain.
...
func (s *Service) buildImport(args *ImportArgs) (*txs.Tx, error) {
	...
	atomicUTXOs, _, _, err := cryft.GetAtomicUTXOs(
		s.vm.ctx.SharedMemory,
		s.vm.parser.Codec(),
		chainID,
		kc.Addrs,
	...
	cryft.SortTransferableOutputs(outs, s.vm.parser.Codec())
...
type ExportArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader
	// Amount of nCRYFT to send
	Amount avajson.Uint64 `json:"amount"`

	// Chain the funds are going to. Optional. Used if To address does not include the chainID.
	TargetChain string `json:"targetChain"`

	// ID of the address that will receive the CRYFT. This address may include the
...
func (s *Service) buildExport(args *ExportArgs) (*txs.Tx, ids.ShortID, error) {
	...
	exportOuts := []*cryft.TransferableOutput{{
		Asset: cryft.Asset{ID: assetID},
		Out: &secp256k1fx.TransferOutput{
