...
import (
	...
	"github.com/shubhamdubey02/cryftgo/vms/avm/block/executor"
	...
	"github.com/shubhamdubey02/cryftgo/vms/components/index"
...
func TestServiceIssueTx(t *testing.T) {
	...
	tx := newTx(t, env.genesisBytes, env.vm.ctx.ChainID, env.vm.parser, "CRYFT")
...
func TestServiceGetTxStatus(t *testing.T) {
	...
	newTx := newCryftBaseTxWithOutputs(t, env.genesisBytes, env.vm.ctx.ChainID, env.vm.TxFee, env.vm.parser)
	txID := newTx.ID()
...
func TestServiceGetAllBalances(t *testing.T) {
	...
	otherAssetUTXO := &cryft.UTXO{
		UTXOID: cryft.UTXOID{
			TxID:        ids.GenerateTestID(),
			OutputIndex: 0,
		},
		Asset: cryft.Asset{ID: otherAssetID},
		Out: &secp256k1fx.TransferOutput{
			Amt: 1337,
			OutputOwners: secp256k1fx.OutputOwners{
				Threshold: 2,
...
func TestServiceGetTxJSON_ExportTx(t *testing.T) {
	...
	newTx := newCryftExportTxWithOutputs(t, env.genesisBytes, env.vm.ctx.ChainID, env.vm.TxFee, env.vm.parser)
	issueAndAccept(require, env.vm, env.issuer, newTx)
...
func TestServiceGetTxJSON_CreateAssetTx(t *testing.T) {
	...
	createAssetTx := newCryftCreateAssetTxWithOutputs(t, env.vm.ctx.ChainID, env.vm.parser)
...
func newCryftBaseTxWithOutputs(t *testing.T, genesisBytes []byte, chainID ids.ID, fee uint64, parser txs.Parser) *txs.Tx {
	cryftTx := getCreateTxFromGenesisTest(t, genesisBytes, "CRYFT")
	key := keys[0]
	tx := buildBaseTx(cryftTx, chainID, fee, key)
	require.NoError(t, tx.SignSECP256K1Fx(parser.Codec(), [][]*secp256k1.PrivateKey{{key}}))
	return tx
}

func newCryftExportTxWithOutputs(t *testing.T, genesisBytes []byte, chainID ids.ID, fee uint64, parser txs.Parser) *txs.Tx {
	cryftTx := getCreateTxFromGenesisTest(t, genesisBytes, "CRYFT")
	key := keys[0]
	tx := buildExportTx(cryftTx, chainID, fee, key)
	require.NoError(t, tx.SignSECP256K1Fx(parser.Codec(), [][]*secp256k1.PrivateKey{{key}}))
	return tx
}

func newCryftCreateAssetTxWithOutputs(t *testing.T, chainID ids.ID, parser txs.Parser) *txs.Tx {
	key := keys[0]
...
func buildBaseTx(cryftTx *txs.Tx, chainID ids.ID, fee uint64, key *secp256k1.PrivateKey) *txs.Tx {
	return &txs.Tx{Unsigned: &txs.BaseTx{
		BaseTx: cryft.BaseTx{
			...
			Ins: []*cryft.TransferableInput{{
				...
				Asset: cryft.Asset{ID: cryftTx.ID()},
...
func buildExportTx(cryftTx *txs.Tx, chainID ids.ID, fee uint64, key *secp256k1.PrivateKey) *txs.Tx {
	return &txs.Tx{Unsigned: &txs.ExportTx{
		...
		ExportedOuts: []*cryft.TransferableOutput{{
			Asset: cryft.Asset{ID: cryftTx.ID()},
			Out: &secp256k1fx.TransferOutput{
				Amt: startBalance - fee,
				OutputOwners: secp256k1fx.OutputOwners{
...
func TestServiceGetUTXOs(t *testing.T) {
	...
	for i := 0; i < numUTXOs; i++ {
		utxo := &cryft.UTXO{
			...
			Asset: cryft.Asset{ID: env.vm.ctx.CRYFTAssetID},
			Out: &secp256k1fx.TransferOutput{
				Amt: 1,
				OutputOwners: secp256k1fx.OutputOwners{
					Threshold: 1,
	...
	tests := []struct {
		...
		},
		{
			...
			expectedErr: cryft.ErrMismatchedChainIDs,
			args: &api.GetUTXOsArgs{
				Addresses: []string{
					xAddr,
					pAddr,
...
func TestCreateFixedCapAsset(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			env := setup(t, &envConfig{
				isCustomFeeAsset: !tc.cryftAsset,
				keystoreUsers: []*user{{
					username:    username,
					password:    password,
					initialKeys: keys,
...
func TestImport(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			utxo := &cryft.UTXO{
				UTXOID: cryft.UTXOID{TxID: ids.Empty},
				Asset:  cryft.Asset{ID: assetID},
				Out: &secp256k1fx.TransferOutput{
					Amt: 7,
					OutputOwners: secp256k1fx.OutputOwners{
