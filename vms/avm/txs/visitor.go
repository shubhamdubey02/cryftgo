...
import "github.com/shubhamdubey02/cryftgo/vms/components/cryft"
...
func (u *utxoGetter) BaseTx(tx *BaseTx) error {
	txID := u.tx.ID()
	u.utxos = make([]*cryft.UTXO, len(tx.Outs))
	for i, out := range tx.Outs {
		u.utxos[i] = &cryft.UTXO{
			UTXOID: cryft.UTXOID{
				TxID:        txID,
...
func (u *utxoGetter) CreateAssetTx(t *CreateAssetTx) error {
	...
	for _, state := range t.States {
		for _, out := range state.Outs {
			u.utxos = append(u.utxos, &cryft.UTXO{
				UTXOID: cryft.UTXOID{
					TxID:        txID,
