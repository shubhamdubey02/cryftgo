...
func TestInitialStateVerifyInvalidOutput(t *testing.T) {
	...
	c := linearcodec.NewDefault()
	require.NoError(c.RegisterType(&cryft.TestState{}))
	m := codec.NewDefaultManager()
	require.NoError(m.RegisterCodec(CodecVersion, c))
	numFxs := 1
	...
	is := InitialState{
		FxIndex: 0,
		Outs:    []verify.State{&cryft.TestState{Err: errTest}},
	}
	err := is.Verify(m, numFxs)
...
func TestInitialStateVerifyUnsortedOutputs(t *testing.T) {
	...
	c := linearcodec.NewDefault()
	require.NoError(c.RegisterType(&cryft.TestTransferable{}))
	m := codec.NewDefaultManager()
	require.NoError(m.RegisterCodec(CodecVersion, c))
	numFxs := 1
	...
	is := InitialState{
		FxIndex: 0,
		Outs: []verify.State{
			&cryft.TestTransferable{Val: 1},
			&cryft.TestTransferable{Val: 0},
		},
	}
	err := is.Verify(m, numFxs)
