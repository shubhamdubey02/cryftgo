...
import (
	...
	blkexecutor "github.com/shubhamdubey02/cryftgoftgo/vms/avm/block/executor"
...
func createTxs() []*txs.Tx {
	return []*txs.Tx{{
		Unsigned: &txs.BaseTx{BaseTx: cryft.BaseTx{
			NetworkID:    constants.UnitTestID,
			BlockchainID: ids.GenerateTestID(),
			Outs: []*cryft.TransferableOutput{{
				Asset: cryft.Asset{ID: ids.GenerateTestID()},
...
func createParentTxs(cm codec.Manager) ([]*txs.Tx, error) {
	...
	for i := 0; i < countTxs; i++ {
		...
		tx := &txs.Tx{Unsigned: &txs.BaseTx{BaseTx: cryft.BaseTx{
			...
			Outs: []*cryft.TransferableOutput{{
				Asset: cryft.Asset{ID: ids.ID{1, 2, 3}},
				Out: &secp256k1fx.TransferOutput{
					Amt: uint64(12345),
					OutputOwners: secp256k1fx.OutputOwners{
						Threshold: 1,
