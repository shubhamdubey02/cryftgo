		...
		BaseTx: cryft.BaseTx{
			...
			Ins: []*cryft.TransferableInput{
				// fee asset
				{
					...
					Asset: cryft.Asset{ID: feeAssetCreateTx.ID()},
				...
				// issued asset
				{
					...
					Asset: cryft.Asset{ID: createTx.ID()},
					In: &secp256k1fx.TransferInput{
						Amt: startBalance,
						Input: secp256k1fx.Input{
							SigIndices: []uint32{
	...
	firstTx := &txs.Tx{Unsigned: &txs.BaseTx{
		BaseTx: cryft.BaseTx{
			...
			Ins: []*cryft.TransferableInput{{
				...
				Asset: cryft.Asset{ID: env.genesisTx.ID()},
				In: &secp256k1fx.TransferInput{
					Amt: startBalance,
					Input: secp256k1fx.Input{
						SigIndices: []uint32{
...
func TestIssueImportTx(t *testing.T) {
	...
	genesisTx := getCreateTxFromGenesisTest(t, env.genesisBytes, "CRYFT")
	cryftID := genesisTx.ID()
	...
	txAssetID := cryft.Asset{ID: cryftID}
	...
	assertLatestIdx(t, env.vm.db, key.PublicKey().Address(), cryftID, 1)
...
func TestClearForceAcceptedExportTx(t *testing.T) {
	...
	key := keys[0]
	assetID := cryft.Asset{ID: cryftID}
	tx := &txs.Tx{Unsigned: &txs.ExportTx{
		BaseTx: txs.BaseTx{BaseTx: cryft.BaseTx{
			NetworkID:    constants.UnitTestID,
			BlockchainID: env.vm.ctx.XChainID,
